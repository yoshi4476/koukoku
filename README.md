...
| 済 | 内容 |
|---|---|
| ✅ | 認証 (メール+パスワード / JWTクッキー / 監査証跡)、オンボーディング初回3ステップ、テナント/クライアント管理 (RLS)、ホーム司令室、統合ダッシュボード (KPI・トレンド・媒体別・キャンペーンドリルダウン)、AI診断、週次レポート (BullMQ自動生成・印刷ビュー・サーバサイドPDF/PPTX)、広告文生成+薬機法/景表法/金商法辞書チェック+文字数検証、CSV取込 (Shift_JIS/UTF-8自動判別)、媒体窓口、異常検知アラート (ルール設定・検知・Slack通知・履歴)、コマンドパレット (⌘K)、LLM原価計測、追記専用監査ログ、媒体API接続基盤 (PlatformConnector・デモ接続・3時間毎同期)、プラン上限制御、承認フロー付き適用 (Phase 3・シミュレーション・承認・実行・ロールバック・kill switch)、CI/vitest |
| ✅ | **強化ロードマップ 9/10施策**: A-1 LLM自己検証パス / A-2 eval回帰自動化 (ゴールデンセット4種・CI組込) / A-3 業種別ベンチマーク / A-4 確信度較正 / B-1 勝ちパターン資産集 (匿名共有・A/B昇格・/copy反映) / B-2 変更履歴タイムライン / B-3 A/Bテスト (z検定) / B-4 予算ペーシング予測 / B-6 Slackスラッシュコマンド |
| ⬜ | **B-5 カスタムダッシュボード** (ドラッグ編集・工数大のため次期)、Google SSO / 2FA、Stripe決済本体、SESメール配信、実媒体API呼出 (GAQL等)、LINE通知 (Messaging API) |
| ⬜ | Google SSO / 2FA、Stripe課金、レポートのメール自動配信 (SES)、媒体API接続 (Phase 2 / 別冊D準拠)、LINE通知 (Messaging API)、承認フロー付き自動適用 (Phase 3) |

> PDF生成はPlaywright (Chromium) を使用。初回のみ `cd apps/api && pnpm exec playwright install chromium` が必要 (未導入時はAPIが導入手順をエラーで案内する)。
