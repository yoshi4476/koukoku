...
http://localhost:3000 を開く。開発中は認証の代わりに `x-tenant-id: t_demo_agency` ヘッダを使用 (webは自動付与)。
...
| 済 | 内容 |
|---|---|
| ✅ | テナント/クライアント/アカウント (RLS)、ホーム司令室 (ルールベースアラート)、統合ダッシュボード (KPI・トレンド・媒体別)、AI診断 (指摘ステータス管理付き)、週次レポート、広告文生成+薬機法/景表法/金商法辞書チェック+媒体別文字数検証、CSV取込 (Shift_JIS/UTF-8自動判別・ヘッダ自動マッピング)、媒体窓口 (付録データ+接続状態)、LLM原価計測 |
| ⬜ | 認証 (Auth.js/Clerk — 現在は x-tenant-id ヘッダ)、Stripe課金、PDF/PPTX出力、媒体API接続 (Phase 2 / 別冊D準拠)、アラート通知 (Slack/LINE)、BullMQ非同期ジョブ化、承認フロー付き自動適用 (Phase 3) |
